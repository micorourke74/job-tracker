...
const STORAGE_KEY = "job_tracker_applications_v3";
const FOCUS_STORAGE_KEY = "job_tracker_current_focus_v1";
const THEME_STORAGE_KEY = "job_tracker_theme_v1";
const DEFAULT_CURRENT_FOCUS = "Help Desk · IT Support · Desktop Support";
...
const WORK_TYPE_OPTIONS = ["On-site", "Hybrid", "Remote"];
const EMPLOYMENT_TYPE_OPTIONS = ["Full-time", "Part-time", "Contract", "Unknown"];
const CLOSED_STATUSES = ["Rejected", "Ghosted", "Withdrawn"];
const ACTIVE_STATUSES = ["Applied", "In Review", "Screening", "Interview", "Assessment", "Offer"];
...
function parsePayNumbers(payRange) {
  if (!payRange) return [];
...
function getEstimatedAnnualPay(job) {
...
function extractJobBasics(text) {
...
function mapImportedJob(headers, row) {
...
function JobModal({ initialJob, onClose, onSave }) {
...
a { color: inherit; }
.app-shell { min-height: 100vh; width: 100%; max-width: 100vw; overflow-x: hidden; background: transparent; color: #172033; transition: background .2s ease, color .2s ease; }
.topbar { position: sticky; top: 0; z-index: 20; padding: 18px 0; background: rgba(255, 255, 255, 0.84); backdrop-filter: blur(16px); border-bottom: 1px solid rgba(148, 163, 184, 0.22); }
...
.about-app-button { border: 1px solid #dbe4ef; background: #fff; color: #263244; border-radius: 12px; padding: 10px 14px; font-weight: 900; font-size: 13px; transition: transform .16s ease, box-shadow .16s ease, background .16s ease; }
.about-app-button:hover { transform: translateY(-1px); box-shadow: 0 12px 22px rgba(15, 23, 42, .08); }
...
.main-layout { width: min(100% - 40px, 1360px); margin: 26px auto 52px; }
.hero-card { display: flex; justify-content: space-between; gap: 34px; align-items: stretch; padding: 34px 30px; border-radius: 28px; background: linear-gradient(135deg, rgba(15, 23, 42, .97), rgba(30, 64, 175, .88)); color: #fff; box-shadow: 0 30px 70px rgba(15, 23, 42, .20); overflow: hidden; position: relative; }
...
.modal-backdrop { position: fixed; inset: 0; z-index: 50; background: rgba(15, 23, 42, .58); backdrop-filter: blur(8px); display: flex; align-items: flex-start; justify-content: center; padding: 28px 16px; overflow-y: auto; }
...
.theme-dark .topbar { background: rgba(15, 23, 42, 0.88); border-bottom-color: rgba(148, 163, 184, 0.18); }
...
.theme-dark .button-secondary, .theme-dark .theme-toggle, .theme-dark .icon-button, .theme-dark .advanced-toggle, .theme-dark .link-pills a, .theme-dark .link-pills button, .theme-dark .row-actions button, .theme-dark .more-menu summary, .theme-dark .tools-menu summary, .theme-dark .tools-menu-list button { background: #172033; color: #e5edf8; border-color: rgba(148, 163, 184, .28); }
